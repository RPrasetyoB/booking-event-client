...
import { getEventsHrApi } from "@/api/event";
...
export const columns = [
  ...
  },
  {
    ...
    header: "Action",
    style: "text-center",
    cell: ({ row, handleDetailOpen, handleDeleteOpen }: any) => {
      return (
...
export function TableEventHr() {
  ...
  const modifiedColumns = columns.map((column) => {
    if (column.cell) {
      return {
        ...column,
        cell: ({ row }: any) =>
          column.cell({ row, handleDetailOpen, handleDeleteOpen }),
      };
    }
    return column;
  });
  ...
  const FetchEventData = useCallback(async () => {
    ...
    try {
      const response = await getEventsHrApi(parsedToken.value);
      if (response.success) {
        setFetching(false);
    ...
    <div className="mx-auto max-w-[1000px] rounded-md bg-card px-4">
      <div className="flex items-center justify-between py-4">
        <Input
          ...
          className="max-w-sm"
        />
        <Button className="btn" onClick={() => setAddNewOpen(true)}>
          Add new booking
        </Button>
      </div>
